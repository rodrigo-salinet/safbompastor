...
if (!isset($index_ok)) { header('Location: ' . $_SERVER['SERVER_NAME']); }
	...
	if ($id_plano == '0') {
		header('Location: ' . $_SERVER['SERVER_NAME'] . '?msg=' . htmlspecialchars('Ops! Plano não encontrado. Por favor, verifique e tente novamente. Obrigado.'));
	}

	$str_sql_plano = "SELECT * FROM planos WHERE ID='$id_plano';";
	$sql_plano = mysqli_query($conexao,$str_sql_plano);

	if (mysqli_num_rows($sql_plano) > 0) {
		mysqli_data_seek($sql_plano,0);

		$linha_plano = mysqli_fetch_assoc($sql_plano);
		$titulo_plano = $linha_plano['TITULO'];
		$titulo_pagina = $titulo_plano;
		$subtitulo_plano = $linha_plano['SUBTITULO'];
		$banner_plano = $linha_plano['LOCAL_IMG_MAIOR'];
		...
		<div id="carousel_banners" class="carousel slide" data-ride="carousel">
			<div class="carousel-inner">
				<div class="carousel-item active">
					<img class="d-block w-100" src="<?php echo $banner_plano; ?>"></a>
				</div>
			</div>
		</div>
	</section>
